...

{
    ...
    dies_ok { csv-write-file(:file($out), @csv) },
      'Dies if no header / accessor list is provided in object mode';
}
...

{
    ...
    $after = slurp $out;
    is($before, $after,
      'Parse / write with provided header / accessor list on pretty.csv ok');
}

{
   class UglyCSV {
    ...
    $after = slurp $out;
    is($before, $after,
      'Parse / write with provided header / accessor list on ugly.csv ok');
}
